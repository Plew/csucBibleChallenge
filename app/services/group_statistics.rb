...
class GroupStatistics
  ...
  def longest_group_streak
    challenge = group.challenge
    group_user_ids = group.users.pluck(:id)
    dates_query = challenge.readings
    dates_query = dates_query.where("scheduled_date >= ?", challenge.stats_start_date) if challenge.stats_start_date.present?
    dates_query = dates_query.where("scheduled_date <= ?", challenge.stats_end_date) if challenge.stats_end_date.present?
    dates = dates_query.order(:scheduled_date).pluck(:scheduled_date).uniq
    max_streak = 0
    current_streak = 0
    dates.each do |date|
      reading = challenge.readings.find_by(scheduled_date: date)
  ...
  def total_chapters_read
    group_user_ids = group.users.pluck(:id)
    query = UserReading.joins(:reading).where(user_id: group_user_ids, readings: { challenge_id: group.challenge_id })
    query = query.where("readings.scheduled_date >= ?", group.challenge.stats_start_date) if group.challenge.stats_start_date.present?
    query = query.where("readings.scheduled_date <= ?", group.challenge.stats_end_date) if group.challenge.stats_end_date.present?
    query.count
  end
