...
RSpec.describe User, type: :model do
  ...
  describe '#qualified_for_banquet?' do
    let(:user) { create(:user) }
    let(:cutoff) { User::BANQUET_CUTOFF_DATE }
    let!(:challenge) { create(:challenge, id: User::BANQUET_CHALLENGE_ID) }

    def complete(reading)
      create(:user_reading, user: user, reading: reading)
    end

    context 'when the user has completed every reading scheduled on or before the cutoff' do
      let!(:on_cutoff) { create(:reading, challenge: challenge, scheduled_date: cutoff) }
      let!(:before_cutoff) { create(:reading, challenge: challenge, scheduled_date: cutoff - 5.days) }

      before { [ on_cutoff, before_cutoff ].each { |r| complete(r) } }

      it 'returns true' do
        expect(user.qualified_for_banquet?).to be(true)
      end

      it 'still returns true when unread readings are scheduled after the cutoff' do
        create(:reading, challenge: challenge, scheduled_date: cutoff + 1.day)
        expect(user.qualified_for_banquet?).to be(true)
      end
    end

    context 'when a reading on or before the cutoff is incomplete' do
      let!(:done) { create(:reading, challenge: challenge, scheduled_date: cutoff - 5.days) }
      let!(:missed) { create(:reading, challenge: challenge, scheduled_date: cutoff - 1.day) }

      before { complete(done) }

      it 'returns false' do
        expect(user.qualified_for_banquet?).to be(false)
      end
    end

    context 'when the challenge has no readings on or before the cutoff' do
      it 'returns false' do
        create(:reading, challenge: challenge, scheduled_date: cutoff + 1.day)
        expect(user.qualified_for_banquet?).to be(false)
      end
    ...
    it 'ignores completions from other challenges' do
      other = create(:challenge)
      other_reading = create(:reading, challenge: other, scheduled_date: cutoff - 1.day)
      create(:reading, challenge: challenge, scheduled_date: cutoff - 1.day)
      complete(other_reading)
      expect(user.qualified_for_banquet?).to be(false)
    end
  end
end
